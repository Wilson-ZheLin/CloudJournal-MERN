...
const Post = ({ post, setCurrentId }) => {
    ...
    const Likes = () => {
      if (post?.likes?.length > 0) {
        return post.likes.find((like) => like === (user?.result?.email || user?.result?._id))
          ? (
        ...
        <Card className={classes.card} raised elevation={4}>
            ...
            {(user?.result?.email === post?.creator || user?.result?._id === post?.creator) && (
              <div className={classes.overlay2}>
                <Button style={{ color: 'white' }} size="large" onClick={() => setCurrentId(post._id)}><EditIcon fontSize="default" /></Button>
              </div>
            )}
